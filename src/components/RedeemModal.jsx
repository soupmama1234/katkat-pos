  ...
  const handleRedeem = async (reward) => {
    ...
    try {
      ...
      const { data: updatedMember, error } = await sb.from("members")
        .update({ points: newPoints, redeemed_rewards: updatedRewards })
        .eq("phone", memberPhone).select().single();
      ...
      <div style={S.modal} onClick={e => e.stopPropagation()}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: "#888", marginTop: 2 }}>{memberInfo?.nickname} · มี <span style={{ color: "#f5c518", fontWeight: "bold" }}>⭐ {currentPoints} แต้ม</span></div>
          </div>
          <button onClick={onClose} style={{ background: "none", border: "none", color: "#666", fontSize: 22 }}>✕</button>
        </div>
        <div style={{ maxHeight: "55vh", overflowY: "auto" }}>
          {loading ? (
            <div style={{ textAlign: "center", color: "#555", padding: 30 }}>กำลังโหลด...</div>
          ) : rewards.length === 0 ? (
            <div style={{ textAlign: "center", color: "#555", padding: 30 }}>ยังไม่มี reward</div>
          ) : rewards.map(r => {
            ...
            return (
              <div key={r.id} style={{ ...S.rewardRow, opacity: canRedeem ? 1 : 0.4 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  {isDiscount && <div style={{ fontSize: 11, background: "rgba(76,175,80,0.15)", color: "#4caf50", display: "inline-block", padding: "1px 6px", borderRadius: 4, marginTop: 2, fontWeight: "bold" }}>ส่วนลด {r.discount_amount}{r.discount_type === "percent" ? "%" : "฿"}</div>}
                  <div style={{ fontSize: 13, color: "#f5c518", marginTop: 4, fontWeight: "bold" }}>⭐ {r.points_required} แต้ม</div>
                </div>
                <button onClick={() => canRedeem && handleRedeem(r)} disabled={!canRedeem || redeeming === r.id} style={{ ...S.btnRedeem, background: canRedeem ? "#f5c518" : "#2a2a2a", color: canRedeem ? "#000" : "#555" }}>{redeeming === r.id ? "⏳" : canRedeem ? "แลกเลย" : "ไม่พอ"}</button>
              </div>
            );
          })}
        </div>
...
const S = {
  ...
  modal: { background: "#1a1a1a", borderRadius: "20px 20px 0 0", padding: "24px 20px", width: "100%", maxWidth: 480, border: "1px solid #2a2a2a" },
  rewardRow: { display: "flex", alignItems: "center", gap: 12, padding: "14px 0", borderBottom: "1px solid #1e1e1e" },
  btnRedeem: { padding: "10px 16px", borderRadius: 10, border: "none", fontWeight: "bold", fontSize: 13, flexShrink: 0, cursor: "pointer" },
};

    ...
    <div style={S.container}>
        ...
        <div style={S.memberSection}>
          {memberStatus === "found" && memberInfo ? (
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: total > 0 ? 8 : 0 }}>
                <div style={{ flex: 1 }}>
                  ...
                  <span style={{ color: "#888", fontSize: 12, marginLeft: 8 }}>⭐ {memberInfo.points} แต้ม</span>
                </div>
              ...
              {(() => {
                ...
                return (
                  <div style={{ marginBottom: 10, padding: "8px 10px", background: "#fff", borderRadius: 10, border: "1px solid #eee" }}>
                    <div style={{ fontSize: 11, color: "#999", fontWeight: "bold", marginBottom: 6 }}>คูปองที่แลกไว้</div>
                    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                      {couponGroups.map((group, idx) => (
                        <div key={idx} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "#f8f9fa", padding: "6px 8px", borderRadius: 8 }}>
                          <span style={{ fontSize: 12, color: "#2e7d32", fontWeight: "bold" }}>🎁 {group.name} {group.count > 1 && `x${group.count}`}</span>
                          <button onClick={() => {
                            ...
                            else addToCart?.({ id: `coupon-${coupon.id}`, name: `🎁 ${coupon.name}`, price: 0, qty: 1, category: "reward", couponId: coupon.id });
                            showToast?.(`ใช้คูปองแล้ว`);
                          }} style={{ background: "#4caf50", color: "#fff", border: "none", borderRadius: 6, padding: "2px 10px", fontSize: 11, fontWeight: "bold" }}>ใช้</button>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <div style={{ display: "flex", gap: 6 }}>
              <input placeholder="ชื่อเล่น" value={regNickname} onChange={e => setRegNickname(e.target.value)} style={{ ...S.input, flex: 1 }} />
              <button onClick={registerMember} style={{ ...S.btnSmall, background: "#2e7d32", color: "#fff" }}>บันทึก</button>
            </div>
          ) : (
            <div style={{ display: "flex", gap: 6 }}>
              <input placeholder="👤 เบอร์สมาชิก" value={memberInput} onChange={e => setMemberInput(e.target.value)} onBlur={e => lookupMember(e.target.value)} style={{ ...S.input, flex: 1 }} />
            </div>
          )}
        </div>
      )}
        ...
        {cart.map((item, index) => (
          <div key={`${item.id}-${index}`} style={S.cartItem}>
            <div style={{ flex: 1 }}>
              ...
              <div style={S.itemDetail}>฿{item.price.toLocaleString()} × {item.qty}</div>
            </div>
      ...
      <div style={S.footer}>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
          <input value={discountInput} onChange={(e) => setDiscountInput(e.target.value)} placeholder="ลด" style={{ ...S.input, flex: 1 }} />
          <button onClick={handleApplyManualDiscount} style={S.btnSmall}>ใช้</button>
        </div>
        <button style={{ ...S.btnPay, backgroundColor: cart.length > 0 ? "#213547" : "#999" }} onClick={() => cart.length > 0 && setShowPayment(true)}>
            ...
            {!isDelivery ? (
              <div style={{ display: "flex", gap: 10, marginBottom: 15 }}>
                {["cash", "promptpay"].map(m => (
                  <button key={m} onClick={() => setPaymentMethod(m)} style={{ ...S.btnMethod, backgroundColor: paymentMethod === m ? "#213547" : "#eee", color: paymentMethod === m ? "#fff" : "#000" }}>{m === "cash" ? "💵 เงินสด" : "📱 โอน"}</button>
                ))}
              </div>
            ) : (
              <input type="text" placeholder={priceChannel === "lineman" ? "เลข 4 หลัก" : "ระบุเลขอ้างอิง"} style={S.inputModal} value={deliveryRef} onChange={e => handleRefChange(e.target.value)} autoFocus />
            )}
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setShowPayment(false)} style={S.btnCancel}>ยกเลิก</button>
              <button disabled={isConfirmDisabled} onClick={handleFinalConfirm} style={S.btnConfirm}>ยืนยัน</button>
            </div>
          </div>
        </div>
      )}
      {showRedeem && memberInfo && <RedeemModal memberPhone={memberPhone} memberInfo={memberInfo} onClose={() => setShowRedeem(false)} onSuccess={() => setShowRedeem(false)} />}
    </div>
  );
}
...
const S = {
  ...
  emptyText: { textAlign: "center", marginTop: "50px", color: "rgba(33,53,71,0.45)" },
  ...
  itemDetail: { fontSize: "13px", color: "#555" },
  qtyControl: { display: "flex", alignItems: "center", backgroundColor: "#f0f0f0", borderRadius: "8px", overflow: "hidden" },
  qtyBtn: { width: "30px", height: "30px", background: "none", border: "none", color: "#213547", fontWeight: "bold", cursor: "pointer" },
  qtyNumber: { minWidth: "25px", textAlign: "center", fontWeight: "bold" },
  ...
  input: { padding: "8px 10px", borderRadius: 8, border: "1px solid #ddd", fontSize: 14, outline: "none" },
  inputModal: { width: "100%", padding: "12px", borderRadius: "8px", border: "1px solid #ddd", fontSize: "18px", textAlign: "center", boxSizing: "border-box", marginBottom: 15 },

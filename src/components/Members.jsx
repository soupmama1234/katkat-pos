...
export default function Members({ orders = [], members: initMembers = [], onMembersChange, showToast, showConfirm, historyTrigger }) {
  ...
  const handleDelete = async (phone) => {
    ...
    const ok = await showConfirm("ลบสมาชิก?", `ยืนยันการลบสมาชิก "${mem?.nickname || phone}"? ข้อมูลจะหายถาวร`);
    if (!ok) return;
    try {
      ...
      <div style={S.content}>
        ...
        {tab === "สมาชิก" && (
          <div>
            ...
            <div style={{ fontSize: 12, color: "#444", marginBottom: 8 }}>แสดง {Math.min(filtered.length, 50)} / {members.length} คน</div>
              ...
              {history.map(h => (
                <div key={h.id} style={S.memberRow}>
                  ...
                  <div style={{ textAlign: "right", display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6 }}>
                    ...
                    <button onClick={() => deleteHistory(h.id)} style={{ background: "none", border: "none", color: "#333", cursor: "pointer", display: "flex", alignItems: "center" }}><Trash2 size={14} /></button>
                  </div>
                </div>
              ))}
            </div>
      ...
      {adjusting && (
        <div style={S.overlay} onClick={() => setAdjusting(null)}>
          <div style={{ ...S.modal, width: 320, textAlign: "left" }} onClick={e => e.stopPropagation()}>
            ...
            <input type="number" placeholder="ใส่จำนวนเอง..." value={adjustVal} onChange={e => setAdjustVal(e.target.value)} style={{ ...S.input, width: "100%", marginBottom: 8 }} />
            <input placeholder="หมายเหตุ (optional)" value={adjustNote} onChange={e => setAdjustNote(e.target.value)} style={{ ...S.input, width: "100%", marginBottom: 14 }} />
...
const S = {
  ...
  btnSave: { background: "#4caf50", border: "none", color: "#fff", borderRadius: 8, padding: "8px 14px", fontSize: 13, cursor: "pointer", fontWeight: "bold", flex: 2 },
